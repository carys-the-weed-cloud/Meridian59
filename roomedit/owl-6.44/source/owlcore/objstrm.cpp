...
ipstream& ipstream::seekg( streamoff off, ios::seekdir dir )
{
  if( good() ){
    objs.RemoveAll();
#if __GNUC__ >=3
//In GCC ios_base::seekdir is deprecated by ios_base::seekdir (both exists but cast is needed)
    streampos p = bp->pubseekoff(off, (ios_base::seekdir)dir, ios::in );
#else
    streampos p = bp->pubseekoff((ios::off_type)off, (ios::seekdir)dir, (ios::open_mode)ios::in );
#endif
    if( p == streampos(EOF) )
      clear( ios::failbit );
  }
...
opstream& opstream::seekp( streampos pos )
{
  if( good() ){
    objs->RemoveAll();
#if __GNUC__ >=3
//In GCC ios_base::seekdir is deprecated by ios_base::seekdir
...
#else
    streampos p = bp->pubseekoff((ios::off_type)off, (ios::seekdir)dir, (ios::open_mode)ios::out );
#endif
    if( p == streampos(EOF) )
      clear( ios::failbit );
  }
