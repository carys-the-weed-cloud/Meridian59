...
{
	long x, xinc, y, yinc;
	int64_t startx, endx, starty, endy, bitmap_width;
...
//	Bool	bClipHaloRight;
	...
	xinc = (bitmap_width << 32) / obj_area->cx;
	yinc = (DibHeight(dos->pdib) << 32) / obj_area->cy;
...
//				bClipHaloRight = TRUE;		//	Not sure why this works.
		...
		row_bits = obj_bits + (y >> 32) * bitmap_width;
			...
			int overlay_depth, AREA *overlay_area)
{
	...
	{
		POINT hotspot_pos;
		int retval = FindHotspot(overlays, pdib_obj, pdib_ov, hotspot, angle, &hotspot_pos);
...
int FindHotspotPdib(PDIB pdib, char hotspot, POINT *point)
{
	for (int i=0; i < DibNumHotspots(pdib); i++)
	{
		int num = DibHotspotNumber(pdib, i);
		if (abs(num) == hotspot)
		{
			POINT p = DibHotspotIndex(pdib, i);
			point->x = p.x;
			point->y = p.y;
